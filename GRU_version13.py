...
# Save the Tokenizer 
tokenizer_file_path = 'Model/GRU/250k_v13_GRU_tokenizer.json' 
...
def create_model(total_words, input_length):
    ...
    model.add(Dense(max(100, total_words // 4), activation='relu')) 
...
plt.tight_layout()
plt.show()
plt.savefig('GRU_v13_training_validation_metrics.png')

#save the model
model.save('Model/GRU/250k_v13_GRU.h5')


model.save('Model/GRU/250k_v13_GRU_savedmodel')


# Save the Tokenizer 
import pickle

tokenizer_file_path = 'Model/GRU/250k_v13_GRU_tokenizer.pkl' 
# Open the file in binary write mode
with open(tokenizer_file_path, 'wb') as file:
    # Serialize the tokenizer object and save it
    pickle.dump(tokenizer, file, protocol=pickle.HIGHEST_PROTOCOL)



# Count total words before removing anything (for total word count)
total_words_count = len(re.findall(r'\b\w+\b', text))
...
# Display the results
print("Total number of words in the dataset:", total_words_count)
print("Total number of unique words:", total_unique_words)
print("Total number of sequences generated:", total_sequences)
...
for _ in range(next_words):
    ...
    print("Word Predicted after this:", seed_text)
